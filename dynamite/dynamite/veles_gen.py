...
class VelesCppGen:
    ...
    def __init__(self, filename, template, forest, data, flat_bundles=True):
        ...
        t = TextRepr.make_text(filename, False)
        ...
        for tree in forest.trees:
            t = TextRepr.make_text(tree.get_name(), False)
            self.text_repr.append(t)
            fun_chunk = Chunk(self.file_chunk, None, None, "FUNCTION", tree.get_name(), t, "")
            self.chunks.append(fun_chunk)
            self.process_block(tree.root, fun_chunk)
