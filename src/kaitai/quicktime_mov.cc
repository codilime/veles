...
quicktime_mov_t::~quicktime_mov_t() { 
    for (std::vector<atom_t*>::iterator it = m_atoms->begin(); it != m_atoms->end(); ++it) {
        delete *it;
    }
    delete m_atoms;
}
...
quicktime_mov_t::atom_t::atom_t(kaitai::kstream *p_io, quicktime_mov_t *p_parent, quicktime_mov_t *p_root) : kaitai::kstruct(p_io) {
    m__io->pushName("_parent");
    m__parent = p_parent;
    m__io->popName();
    m__io->pushName("_root");
